...
For a closed system whose internal interactions obey Newton's third law, internal forces appear in third-law pairs, so their vector sum is zero. This is a very powerful result. 
...
Using $\vec{F}_{ji}=-\vec{F}_{ij}$, the paired torque is $(\vec{r}_i-\vec{r}_j)\times\vec{F}_{ij}$. It vanishes when the internal force is central, meaning parallel to the separation vector.
