...
# Unit 5 - Notes: Conservation of Energy
...
A useful account of these situations is [*Pseudowork and real work*, American Journal of Physics](https://pubs.aip.org/aapt/ajp/article-abstract/51/7/597/1052185/Pseudowork-and-real-work?redirectedFrom=fulltext), which discusses limitations of the point-particle model.
:::
...
![Work done by a net force](../images/notes/unit5/discrete-force-intervals.png)
...
$$W_{\text{net}} = \sum_{i=0}^{n-1} F_{net,i}\Delta x_i = \Delta T.$$
...
$$\lim_{n \rightarrow \infty} \sum_{i=0}^{n-1} F_{net,i}\Delta x_i = \int_{x_0}^{x_n} F_{net}(x)dx.$$
...
![Work done by a net force](../images/notes/unit5/path-integral-work.png)


The work done by the net force is:

$$W_{\text{net}} = \sum_{i=0}^{n-1} \vec{F}_{net,i}\cdot\Delta \vec{r}_i.$$
...
A less obvious example that produces a potential energy function is a [lattice chain](https://en.wikipedia.org/wiki/Lattice_chain). Here we model an electron moving in 1D near but not too near a long chain of atoms. The picture below shows the model.

![Lattice chain model](../images/notes/unit5/lattice-chain.png)

Here the location of the particle and its initial velocity are zero. The force model for a chain of atoms in this arrangement is:
...
The expression under the square root must be nonnegative for an allowed trajectory. In particular, the stated initial condition $x=0$, $v=0$ is an equilibrium, so the particle remains at rest unless it is perturbed. For a physically accessible displacement, use the energy relation above with an appropriate initial condition.
...
3. if the curl of the force is zero throughout a simply connected domain, then the force is conservative.

With the usual smoothness assumptions and a simply connected domain, these statements are equivalent. Without the domain assumption, zero curl alone need not imply a globally defined potential.
...
![Work done by a net force](../images/notes/unit5/closed-path-work.png)
