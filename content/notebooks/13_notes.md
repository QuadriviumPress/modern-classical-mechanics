...
# Unit 13 - Notes: Examples of Lagrangian Mechanics
...
![Atwood Machine](../images/notes/unit13/atwood.png)
...
![Unraveled String](../images/notes/unit13/string-unraveled.png)
...
In the previous example, we didn't take into account the energy needed to rotate the pulley. Let's do that now. Because the rope cannot slip, any small rotation $R d\phi$ of the pulley gives a change $dy_1$ in the position of mass $M$. This is the [no slip constraint](https://en.wikipedia.org/wiki/No-slip_condition).
...
This equals the generalized momentum associated with the pulley angle. The hanging masses move vertically, not tangentially around the pulley, so their contributions should be computed from their actual vertical position vectors and velocities rather than from $R\hat r$ and $R\dot\phi\hat\phi$.

$$\vec{L}_{total} = \vec{L}_{disk} + \vec{L}_{M} + \vec{L}_{m}$$

$$\vec{L}_{disk} = I_{disk}\omega = \dfrac{1}{2}M_p R^2 \dot{\phi}\,\text{(out of the page)}$$

$$\vec{L}_{M} = M\vec{r}_{M}\times \vec{v}_{M}\quad\text{(computed using the actual vertical position and velocity)}$$
...
$-(M-m)gR - \dfrac{d}{dt}\left((M+m+\frac{1}{2}M_p)R^2\dot{\phi}\right)=0$
...
![Bead in a Parabolic Bowl](../images/notes/unit13/paraboloid.png)
...
$$\mathcal{L}(\rho, \dot{\rho}, \dot{\phi}) = \dfrac{1}{2}m\left((1+c^2\rho^2)\dot{\rho}^2 + \rho^2\dot{\phi}^2\right) - mg\dfrac{1}{2}c\rho^2$$
...
$$\dfrac{\partial \mathcal{L}}{\partial \rho} = m\rho\dot{\phi}^2 + mc^2\rho\dot{\rho}^2 - mgc\rho$$

$$\dfrac{\partial \mathcal{L}}{\partial \dot{\rho}} = m(1+c^2\rho^2)\dot{\rho}$$

$$\dfrac{d}{dt}\left(\dfrac{\partial \mathcal{L}}{\partial \dot{\rho}}\right) = m(1+c^2\rho^2)\ddot{\rho} + 2mc^2\rho\dot{\rho}^2$$

We can now write the equation of motion:

$$m\rho\dot{\phi}^2 + mc^2\rho\dot{\rho}^2 - mgc\rho - m(1+c^2\rho^2)\ddot{\rho} - 2mc^2\rho\dot{\rho}^2 = 0$$

We can clean this up a little bit:

$$\ddot{\rho}(1+c^2\rho^2) + c^2\rho\dot{\rho}^2 = \rho\dot{\phi}^2 - gc\rho$$

$$\ddot{\rho}(1+c^2\rho^2) + c^2\rho\dot{\rho}^2 - \rho\dot{\phi}^2 + gc\rho = 0$$

We try to write both equations in terms of their accelerations. We have:

$$\ddot{\rho} = -\dfrac{c^2\rho\dot{\rho}^2 - \rho\dot{\phi}^2 + gc\rho}{1+c^2\rho^2}$$
