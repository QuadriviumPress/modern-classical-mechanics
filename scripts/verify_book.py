...
Adapted for notebook-first layouts (``content/`` + Jupytext frontmatter): TOC
parsing ignores ``exports``/``downloads``, labels ignore YAML ``name:`` keys,
and image scans include ``content/images/``.
...
def toc_files(myst_text: str) -> list[str]:
    """Return ``file:`` entries under ``project.toc`` only (not exports/downloads)."""
    files: list[str] = []
    in_toc = False
    for line in myst_text.splitlines():
        ...
        if not in_toc:
            continue
        if re.match(r"^[^\s#]", line) or re.match(r"^  [A-Za-z0-9_]+:\s*", line):
            break
...
def content_paths(listed: list[str]) -> list[Path]:
    ...
    for directory in ("chapters", "front", "back", "appendices", "content"):
        folder = ROOT / directory
        if not folder.is_dir():
            continue
        # Nested layouts (e.g. content/notebooks/) need a recursive walk.
        matcher = folder.rglob("*.md") if directory == "content" else folder.glob("*.md")
        paths.extend(sorted(matcher))
...
def main() -> int:
    ...
    texts = {path: strip_frontmatter(path.read_text(encoding="utf-8")) for path in paths}
    ...
    image_dirs = []
    for candidate in (ROOT / "images", ROOT / "content" / "images"):
        if candidate.is_dir():
            image_dirs.append(candidate)
    for folder in ROOT.glob("**/images"):
        if folder.is_dir() and folder not in image_dirs:
